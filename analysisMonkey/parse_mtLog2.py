...
@author: shwang
@version: 6.0
@changes: Parse the current activity and combine to the total activity for data summary
...
def parse_monkeyLog_crash(filePath):   
    ...
    for lineNumb, line in enumerate(fh):
        ...
        if "// CRASH: com.sgiggle" in line:
            ...
            history = fh[lineNumb-180:lineNumb]
            flag = True
            acts = []
            for i in range(len(history)-1,0,-1):
                if ":Current Activity:" in history[i]:
                    act = history[i].split(":Current Activity:",1)[1].strip()
                    if act:
                        acts.append(act)
                    if flag:
                        fl.append("\n\n[Events to trigger this crash]\n")
                        fl.append("".join(history[i:lineNumb]))
                        flag = False
                        
            if acts:
                #print acts
                fl.append("\n[Last several activities by time ASC]:\n")
                acts = acts[::-1]
                #print acts
                if len(acts) <= 5:
                    ...
                    for i in range(5):
                        ...
                        fl.append("{}. {}\n".format(i+1, acts[len(acts)-5+i]))
                    #fl.append("\n".join(acts[:5]))
            
            fl.append("\n\n[Crash stack]:\n")        
            fl.append(line)
            for fline in fh[lineNumb+1:lineNumb+180]:
...
def get_activities(filePath):
    ...
    current_actList = get_current_activity(filePath)
    if current_actList:
        actList += current_actList
...
def get_current_activity(filePath):
    # parse all allowed activities
    actList = []
    #if os.path.isfile(filePath):
    if os.path.exists(filePath):
    ...
    for line in fh:
        if ":Current Activity:" in line:
            act = line.split(":Current Activity:",1)[1].strip()
            if act:
                actList.append(act)
...
if __name__ == "__main__":
    ...
    #print parse_monkeyLog_crash(logFile)

   
    """
    actList = get_current_activity(logFile)
